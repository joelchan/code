...
- Configurable
  ...
  https://github.com/fmalina/transcript/blob/master/transcript.py
"""
...
REPLACE_AFTER = ()
HTML_DIR = 'E:\\code\\cermine\\processed_pdfs\\test'
...
def remove_headers(dom):
    ...
    for n1 in dom.cssselect(
            '.pc > *:first-child'):  # for each 1st element on page
        n1_y = classN('y', n1)  # get its top position
        topmost = parent(n1).cssselect(
            '.y' + n1_y)  # select those with the same top position
    ...
    if len(texts) > 1 and all(x == texts[-1] for x in texts[
                                                      1:]):  # if they are all the same from 2nd page
...
def grid_data(dom, get_dimension):
    ...
    for l in dom.cssselect('.t'):
        ...
        cb = parent(l) if parent(l).attrib.get('class', '').startswith(
            'c ') else None
        ...
        data.append(
            types.SimpleNamespace(page=page, x=x, y=y, elem=l, clipbox=cb,
                                  lines=[], text=l.text)
        )
    return data
...
def reconstruct_tables(dom, data):
    ...
    for e in dom.iter():
        if e.tag in (
        'span', 'div') and not e.text_content() or e.text_content() == ' ':
            e.drop_tag()
...
def prepare(doc_path):
    ...
    css = open(doc_path.replace('.html', '.css'), 'rt',
...
def heading_levels(dom, dimensions):
    ...
    fs_stats = [(len(dom.cssselect('.fs' + cssn)), cssn, fs) for cssn, fs in
                dimensions['fs'].items()]
...
def semanticize(doc_path='test.html'):
    ...
    for l in dom.cssselect('.t'):
        ...
        look = ' '.join([c for c in l.attrib['class'].split() if
                         c[0] != 'y' and c[
                                         0:2] != 'fc'])  # ignore y pos and font color
        ...
        if DEBUG:
            ...
            print(' Aa %d    ⇪ %d    ⇕ % 3d    %s    %s    %s' % \
                  (new_look, p_space, line_height, l.attrib['class'].ljust(40),
                   mark, txt))
    ...
    #         s = re.sub(rm, '', s)
    return s
    # save_path = os.path.dirname(doc_path.replace('html', 'htm'))
...
if __name__ == '__main__':
    ...
    path = 'E:\\code\\cermine\\processed_pdfs\\HTML\\Sensemaking_tools_for_understanding_research_literatures.html'
    test = semanticize(doc_path=path)
    print(test)